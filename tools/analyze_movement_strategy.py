...
def save_figures(events: pd.DataFrame, assignments: pd.DataFrame, centroids: pd.DataFrame) -> None:
    ...
    for cluster, group in assignments.groupby("cluster"):
        ...
        for row in group.itertuples():
            ax.annotate(f"{row.video_id}-S{row.set_id}-{row.player}", (row.pca_1, row.pca_2), xytext=(3, 3), textcoords="offset points", fontsize=7)
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(plot.index, rotation=40, ha="right")
