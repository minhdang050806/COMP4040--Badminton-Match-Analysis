...
def save_workflow() -> None:
    ...
    for label, (x, y), color in boxes:
        ax.text(
            ...
            bbox={"boxstyle": "round,pad=0.48", "fc": color, "ec": "#365f7d", "lw": 1.4},
        )
    ...
    for source, target in arrows:
        ...
        ax.annotate(
            ...
            arrowprops={"arrowstyle": "->", "lw": 1.4, "color": "#365f7d", "shrinkA": 46, "shrinkB": 46},
        )
    ax.text(0.02, 0.96, "Model development", fontsize=12, fontweight="bold", color="#274f6b")
    ax.text(0.02, 0.62, "Unseen-video inference", fontsize=12, fontweight="bold", color="#365f35")
    ax.text(0.02, 0.18, "Analytical outputs", fontsize=12, fontweight="bold", color="#795d18")
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    fig.tight_layout()
...
def save_stroke_distribution(predictions: pd.DataFrame) -> None:
    ...
    ax.set_ylim(0, 1)
    ax.legend(title="Stroke type", bbox_to_anchor=(1.01, 1), loc="upper left", fontsize=8)
    fig.tight_layout()
...
def save_cluster_projection(units: pd.DataFrame) -> None:
    ...
    for cluster, group in units.groupby("cluster"):
        ...
        for row in group.itertuples():
            ax.annotate(f"{row.video_id}-{row.predicted_player_side}", (row.pca_1, row.pca_2), xytext=(4, 4), textcoords="offset points", fontsize=8)
...
def save_quality_accuracy() -> None:
    ...
    ax.bar_label(bars, labels=[f"{value:.1%}" for value in selected["accuracy"]], padding=3)
