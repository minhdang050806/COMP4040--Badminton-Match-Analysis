...
def plot_matrix(matrix: np.ndarray, states: list[str], title: str, path: Path, cmap: str = "viridis") -> None:
    fig, ax = plt.subplots(figsize=(max(10, len(states) * 0.62), max(9, len(states) * 0.62)))
    image = ax.imshow(matrix, cmap=cmap, aspect="auto")
    ax.set_xticks(range(len(states)), states, rotation=90, fontsize=12)
    ax.set_yticks(range(len(states)), states, fontsize=12)
...
def plot_accuracy_diagnostics(pred: pd.DataFrame, figures: Path) -> None:
    ...
    for row in videos.itertuples():
        ax.annotate(str(row.video_id), (row.top_pose_valid, row.accuracy), xytext=(6, 6), textcoords="offset points", fontsize=13)
