...
from model.protogcn import ProtoGCN, ContrastiveLoss
...
def set_loss_fn(
    ...
    contrastive = ContrastiveLoss(n_class, n_channel).to(device)
    
    if weight_path is None:
        contrastive.train()
    else:
        contrastive.load_state_dict(torch.load(weight_path, map_location=device, weights_only=True))
        contrastive.eval()
    
    loss_fn = lambda logits, f, labels: \
        cross_entropy(logits, labels) + alpha * contrastive(logits, f, labels)
    
    return loss_fn, contrastive
...
def train_one_epoch(
    ...
    for (human_pose, pos, shuttle), video_len, labels in loader:
        ...
        # human_pose: (n, t, m=2, v, c)
        human_pose = human_pose.permute(0, 2, 4, 1, 3).contiguous()
        # human_pose: (n, m, c, t, v)
        logits, z = model(human_pose)
        loss: Tensor = loss_fn(logits, z, labels)
...
def train_network(
    ...
    loss_fn, contrastive_model = set_loss_fn(
        device=device,
        n_class=hyp.n_classes,
        n_channel=(hyp.V * hyp.V),
    )
    
    optimizer = optim.AdamW(chain(model.parameters(), contrastive_model.parameters()), lr=hyp.lr)
...
if __name__ == '__main__':
    ...
    additional_model_info = 'label_smoothing'
    ...
    for serial_no in range(1, 6):
        ...
        task.get_network_architecture(model_name='ProtoGCN', in_channels=(3 if hyp.use_3d_pose else 2))
