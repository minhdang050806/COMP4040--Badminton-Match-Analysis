...
from preparing_data.tenniSet_dataset import prepare_npy_collated_loaders, \
                                            RandomTranslation_batch, \
                                            get_stroke_types, get_bone_pairs
...
class Task:
    ...
    def seek_network_weights(self, model_info='', serial_no=1):
        ...
        if '_' in self.model_name:
            first_name, last_name = self.model_name.split('_', 1)
            save_name = first_name.lower() + '_' + last_name + model_postfix
        else:
            save_name = self.model_name.lower() + model_postfix
        self.model_name += model_postfix

        weight_path = Path(f'weight_on_tenniSet/{save_name}.pt')
...
if __name__ == '__main__':
    # Train and test on TenniSet data
    strokes_info_dir = Path('../TenniSet')
    ...
    for serial_no in range(1, 11):
        ...
        task.prepare_dataloaders(
            root_dir=Path(f'preparing_data/TenniSet_data')\
                         /'dataset_npy_collated',
            pose_style=hyp.pose_style,
        )
        task.get_network_architecture(model_name='BST', in_channels=2)
